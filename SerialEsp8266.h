...
private:
        ...
        long refresh_period = 350; //5 minutos
        /* 
        ...
        */
        ...
        void handleProtocolWithEsp(String command)
        {
                ...
                if (command.indexOf("ACS_G:off") >= 0)
                {
                        Flag_ACS_EN = false;
                }else
                if (command.indexOf("ACS_DT_E:on") >= 0)
                {
                        Flag_ACS_DT_EN = true;
                }else
                if (command.indexOf("ACS_DT_E:off") >= 0)
                {
                        Flag_ACS_DT_EN = false;
                }else
                if (command.indexOf("ACS_E:on") >= 0)
                {
                        Flag_ACS_EN_ELECT = true;
                }else
                if (command.indexOf("ACS_E:off") >= 0)
                {
                        Flag_ACS_EN_ELECT = false;
                }else
                if (command.indexOf("ALARM_RESET") >= 0)
                {
                        resetAlarms();
                }else
                if (command.indexOf("MODO_FRIO:on") >= 0)
                {
                        modoFrio = true;
                        changeModo();
                }else
                if (command.indexOf("MODO_FRIO:off") >= 0)
                {
                        modoFrio = false;
                        changeModo();
                }else                
                if (command.indexOf("TEMP_ACS:") >= 0)
                {
                        uint8_t aux = command.substring(9, command.length() - 1).toInt();
